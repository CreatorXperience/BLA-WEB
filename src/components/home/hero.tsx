    ...
    const slides = hero
      .map((s) => {
        const section = s as {
          title?: string;
          subtitle?: string;
          content?: { ctaText?: string; ctaUrl?: string; mediaUrl?: string };
        };
        return {
          id: (s as { id?: string }).id ?? Math.random().toString(36),
          image: section.content?.mediaUrl ?? IMAGERY.hero[0],
...
export function Hero() {
  ...
  return (
    <section className="relative h-[92vh] min-h-[560px] w-full overflow-hidden bg-ink" aria-label="Featured">
